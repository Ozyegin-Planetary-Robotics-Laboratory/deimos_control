...
public:
  ...
  */
   ...
   * @param pose The position to bring the motor to. (degrees)
   * 
   * @param vel The velocity to move the motor with. (degrees/sec)
   * 
   * @param acc The acceleration to move the motor with. (degrees/sec^2)
  */
  void sendPositionVelocityAcceleration(float pose, int16_t vel, int16_t acc) {
    if (_can_fd < 0) {
      return;
    }
    static const int16_t acc_max = 200;
    static const int16_t acc_min = 0;
